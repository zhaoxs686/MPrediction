...
def main(config):
    ...
    test_dataset = BERTDataset(corpus_path=config["test_smile"],
                               corpus_label=config["test_label"],
    ...
    train_dataset = BERTDataset(corpus_path=config["train_smile"],
                               corpus_label=config["train_label"],
                                 word2idx_path=config["word2idx_path"],
                                seq_len=config["max_seq_len"],
                                hidden_dim=config["hidden_size"],
                                )
    ...
    checkpoint = torch.load(f'./models/pretain2.pt',
                            map_location=device)
    model.load_state_dict(checkpoint)
    model.transform1 = nn.Linear(model.transform1.in_features, 128).to(device)
    model.transform3 = nn.Dropout(p=0.3)
    model.transform4 = nn.Linear(128, 2).to(device)

    optimizer = torch.optim.Adam([{'params': model.transform1.parameters(), 'lr': config["lr"]},
                                  {'params': model.transform4.parameters(), 'lr': config["lr"]}],weight_decay=0.0001)
    # optimizer = torch.optim.Adam(model.parameters(), lr=config["lr"],weight_decay=1e-4)
    loss_func = torch.nn.CrossEntropyLoss(weight=torch.tensor(train_dataset.weight).to(device))  # 忽视一些填充
    ...
    with torch.no_grad():
        for step, data in enumerate(test_dataloader):
            ...
            final, Gembedding, Bembedding, logit = model(g_train, bert_input, pos_embedding)
            logit = m(logit).cpu().numpy()

    for i in range(len(tlabel)):
        print(logit[i, 1], tlabel[i].numpy())
...
if __name__ == '__main__':
    ...
    # 配置bert模型参数
    config = {}
    config["test_smile"] = "./data/newcom_smiles.txt"
    config["test_label"] = "./data/newcom_label.mat"
    config["train_smile"] = "./data/train_smiles.txt"
    config["train_label"] = "./data/train_label.mat"
